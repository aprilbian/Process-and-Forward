...
job_name = 'JSCC_full_duplex_transformer_648_'+args.channel_mode +'_relay_' + args.relay_mode +'_dataset_'+str(args.dataset)+'_cout_'+str(args.cout)+'_layers_'+str(args.layers)+\
            '_unit_'+str(args.unit) + '_gamma1_' + str(args.gamma1) + '_gamma2_' + str(args.gamma2) + '_unit_trans_feat_' + str(args.unit_trans_feat)\
                 + '_hidden_size_' + str(args.hidden_size) + '_n_heads_' + str(args.n_heads) + '_n_layers_' + str(args.n_layers)

if args.adapt:
    job_name = job_name + '_gamma_rng_' + str(args.gamma_rng) + '_P_rng_' + str(args.P_rng)
...
args_copy.n_layers = 6
source_enc = tf_encoder(args_copy).to(args.device)

# revise the args for relay_enc
args_copy = copy.deepcopy(args)
args_copy.n_layers = 4
args_copy.n_feat = 2*args.n_trans_feat
args_copy.n_trans_feat = args.unit_trans_feat
relay_enc = tf_encoder(args_copy).to(args.device)

source_dec = tf_decoder(args).to(args.device)

jscc_model = RelayPFRD_Transformer_Dualinput_Full_Adapt(args, source_enc, source_dec, relay_enc)
